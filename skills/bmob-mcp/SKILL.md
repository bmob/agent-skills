...
> **工具数量**：`tools/list` 返回 **10 个**；agent **调用 9 个**（不含内部工具 `mcp_endpoint_mcp_post`）。
...
## 10 个工具的真实 schema（9 个供 agent 调用）

`tools/list` 共 10 个；**不要调用**第 10 个 `mcp_endpoint_mcp_post`（服务端内部回环）。下方 1–9 为 agent 可用工具。
