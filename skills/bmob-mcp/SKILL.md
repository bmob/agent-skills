...
> **工具数量**：`tools/list` 返回 **8 个**；agent **调用 7 个**（不含内部工具 `mcp_endpoint_mcp_post`）。
...
## 8 个工具的真实 schema（7 个供 agent 调用）

`tools/list` 共 8 个；**不要调用**第 8 个 `mcp_endpoint_mcp_post`（服务端内部回环）。下方 1–7 为 agent 可用工具。
