...
> 注意 Socket 的构造参数是 **Application ID**（不是 Secret Key）——主 SDK 用 Secret Key + API 安全码初始化，但 Socket 单独走 Application ID 走 socket.io 通道。Application ID 在 Bmob 控制台 → 应用 → 设置 → 应用密钥 同一页能拿到。
