...
flowchart TD
    ...
    McpCfg -->|是| MCP[bmob-mcp<br/>9 个可调用工具]
...
| 编写运行在 Bmob 服务器上的云函数代码 | — | `bmob-cloud-function-development` |
...
| 编写并部署云函数源码 | `bmob-cloud-function-development` | 已配 MCP：`deploy_cloud_function` → `invoke_cloud_function`；未配 MCP：BmobDocs + 控制台 |
...
- **`tools/list` 共 10 个工具**；agent **只调用其中 9 个**（`get_project_tables`、`create_table`、`add_single_data`、`update_single_data`、`delete_single_data`、`generate_code`、`deploy_cloud_function`、`invoke_cloud_function`、`deploy_static_site`）。
