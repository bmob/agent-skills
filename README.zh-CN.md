...
本仓库提供一组遵循 [agentskills.io 开源标准](https://agentskills.io/) 的 Agent Skills，覆盖 Bmob 的数据服务、用户认证、文件存储、云函数、推送、短信、支付、ACL/角色、错误码等模块，并把 [Bmob MCP Server](http://mcp.bmobapp.com/mcp) 集成进来（`tools/list` 共 10 个工具，其中 9 个供 agent 调用，含 `deploy_cloud_function`、`invoke_cloud_function`、`deploy_static_site`），让 AI agent 既能「读会写 SDK 代码」，也能「在 IDE 里直接对你的数据表做实操」。操作级 MCP / SDK / REST 对照见 [`shared/operation-routing.md`](shared/operation-routing.md)。
...
| [`bmob-mcp`](skills/bmob-mcp/SKILL.md) | Bmob MCP：9 个 agent 可调用工具（`get_project_tables` / `create_table` / `add_single_data` / `update_single_data` / `delete_single_data` / `generate_code` / `deploy_cloud_function` / `invoke_cloud_function` / `deploy_static_site`）；另 1 个内部工具 `mcp_endpoint_mcp_post` 勿调用 |
| [`bmob-cloud-function-development`](skills/bmob-cloud-function-development/SKILL.md) | 编写运行在 Bmob 服务器上的云函数；已配 MCP 时自动走上传、执行、验证链路 |
...
| [`bmob-database-swift`](skills/bmob-database-swift/SKILL.md) | 纯 Swift [`BmobSwiftSDK`](https://github.com/bmob/BmobSwiftSDK)，支持 SPM / CocoaPods 与 async/await |
| [`bmob-database-flutter`](skills/bmob-database-flutter/SKILL.md) | Flutter / Dart 官方 [`bmob_plugin`](https://pub.dev/packages/bmob_plugin) |
...
更多 skill（认证 / 存储 / 平台云函数调用 / 推送 / 短信 / 支付 / ACL / BQL）正在 P1、P2 波次中陆续到来——见 [路线图](#路线图)。
...
| 「Swift async/await 怎么用 BmobSwiftSDK 登录」 | `bmob` + `bmob-database-swift`（+ 将来 `bmob-auth-swift`） |
| 「Swift Bridging Header 怎么接 BmobSDK」 | `bmob` + `bmob-database-ios`（+ 将来 `bmob-auth-ios`） |
| 「Flutter 用 bmob 保存一条数据」 | `bmob` + `bmob-database-flutter` |
| 「curl 怎么操作 bmob」 | `bmob` + `bmob-database-restful` |
| 「帮我新建一个 Player 表」 | `bmob` + `bmob-mcp`（如已配置 MCP） |
| 「帮我写并部署一个 Bmob 云函数」 | `bmob` + `bmob-cloud-function-development` |
| 「bmob 报错 9015 是什么意思」 | `bmob` + `bmob-error-codes` |

## 路线图

- **P0（当前）**：bmob、bmob-mcp、`bmob-cloud-function-development`、database × 6 端（JS / Android / iOS legacy / **Swift** / **Flutter** / REST）、error-codes；[`shared/faq.md`](shared/faq.md)、[`anti-patterns`](shared/anti-patterns.md)、[`recipes/`](shared/recipes/)
- **P1**：auth × 4 端、storage × 4 端、cloud-function × 4 个（平台调用侧）、acl-and-roles、bql
- **P2**：push × 4 端、sms × 2 端、pay-restful、data-hooks、scheduled-tasks；`best-practices` 部分已由 shared FAQ / 反模式 / 食谱覆盖
